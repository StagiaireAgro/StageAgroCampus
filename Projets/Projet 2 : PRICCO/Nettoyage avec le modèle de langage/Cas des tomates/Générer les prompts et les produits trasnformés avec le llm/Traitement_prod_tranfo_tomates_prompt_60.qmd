---
title: "Traitement_prod_tranfo_tomates_prompt_60"
format: html
---
...
vect_verifi_LLM_res_tomates_60_1 <- lapply(prompts_60_levels_tomates_2, extraire_liste_produits)
...
resultats_comparaison <- comparer_listes_produits(prompts_60_levels_tomates_2, res_60)
View(resultats_comparaison)




#produits_transfo_llm <- unlist(lapply(resultats_comparaison, function(x) x$levels_llm))

#fausses_hallucina_llm <- unlist(lapply(resultats_comparaison, function(x) x$fausses_hallucina))

#vraies_hallucina_llm <- unlist(lapply(resultats_comparaison, function(x) x$vraies_hallucina))

#levels_communs_llm <- unlist(lapply(resultats_comparaison, function(x) x$levels_communs))
...
table_indica_purete_asperge <- data.frame(chunck_traite = c(1:length(resultats_comparaison)),
...
data_total_indicat_purete <-
  data.frame("Totale",sum(table_indica_purete_asperge$nb_levels_initial),sum(table_indica_purete_asperge$nb_levels_llm),sum(table_indica_purete_asperge$nb_levels_commun),sum(table_indica_purete_asperge$nb_fausses_halluci_llm),sum(table_indica_purete_asperge$nb_vraies_halluci_llm))
...
data_moy_indicat_purete <-
  data.frame("Moyenne",NA,round(mean(table_indica_purete_asperge$nb_levels_llm)),round(mean(table_indica_purete_asperge$nb_levels_commun)),round(mean(table_indica_purete_asperge$nb_fausses_halluci_llm)),round(mean(table_indica_purete_asperge$nb_vraies_halluci_llm)))
...
# VII) Suppresion des levels de produits transforéms dans le jeu de données

```{r}
# Importation du jeu de données des asperges
data_tomate <- read_excel("data_tomate.xlsx")
