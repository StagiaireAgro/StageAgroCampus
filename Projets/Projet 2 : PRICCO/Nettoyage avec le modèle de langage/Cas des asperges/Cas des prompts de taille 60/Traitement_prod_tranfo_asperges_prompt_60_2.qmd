...
comparer_listes_produits <- function(prompts_attendus, reponses_llm, verite_terrain) {
  ...
  # S'il y a plusieurs noms de produit
  nom_llm <- tolower(h)
  
  # rechercher une correspondance exacte (en minuscules)
  correspondance_exacte <- vrais_noms[tolower(vrais_noms) == nom_llm]
  
  if (length(correspondance_exacte) == 1) {
    return(correspondance_exacte)
  } else {
    # Sinon, on choisit celui avec la plus petite distance de Levenshtein
...
table_indica_purete_asperge <- data.frame(
  ...
  nb_prod_llm_st_prod_transfo = NA)
...
table_indica_purete_asperge$nb_prod_llm_st_prod_transfo <- sapply(resultats_comparaison, function(x) (length(x$levels_communs) - length(x$produit_transfo_llm)))


### Calcul des moyennes et totaux

# Selectionner les noms des colonnes en retirant la colonne "chunck_traite" 
cols_to_sum <- names(table_indica_purete_asperge)[-1]

# Calcul des totaux et moyennes par colonne
totaux <- colSums(table_indica_purete_asperge[, cols_to_sum], na.rm = TRUE)
moyennes <- colMeans(table_indica_purete_asperge[, cols_to_sum], na.rm = TRUE)
...
# Ajout au tableau 
table_indica_purete_asperge <- rbind( table_indica_purete_asperge,
                                      ligne_total, ligne_moyenne)

# Reconversion des colonnes sauf 'chunck_traite' en numérique
table_indica_purete_asperge[cols_to_sum] <- lapply(table_indica_purete_asperge[cols_to_sum], as.numeric)

# On remplace la moyenne de levels_initial par un NA car ça n'a pas de sens
table_indica_purete_asperge[length(resultats_comparaison)+2,2] <- NA
...
View(matrice_confusion_prompt_60)
```
...
# Etape 1 : Vrais positifs : Les produits transformés trouvés par le llm qui sont des produits transformés 
matrice_confusion_prompt_60$Positifs[[1]] <- table_indica_purete_asperge$nb_prod_transfo_trouves_llm[[length(resultats_comparaison)+1]]

# Etape 2 : Faux positifs : Les produits transformés trouvés par le llm qui sont des produits bruts
matrice_confusion_prompt_60$Negatifs[[1]] <- table_indica_purete_asperge$nb_levels_commun[[length(resultats_comparaison)+1]] - table_indica_purete_asperge$nb_prod_transfo_trouves_llm[[length(resultats_comparaison)+1]]

# Etape 3 : Vrais négatifs : Les produits bruts trouvés par le llm qui sont des produits bruts 
matrice_confusion_prompt_60$Negatifs[[2]] <- table_indica_purete_asperge$nb_levels_initial[length(resultats_comparaison)+1] - table_indica_purete_asperge$nb_levels_commun[[length(resultats_comparaison)+1]] - table_indica_purete_asperge$nb_prod_transfo_pas_trouves_llm[[length(resultats_comparaison)+1]]

# Etape 4 : Faux négatifs : Les produits bruts trouvés par le llm qui sont des produits transformés 
matrice_confusion_prompt_60$Positifs[[2]] <-table_indica_purete_asperge$nb_prod_transfo_pas_trouves_llm[[length(resultats_comparaison)+1]]
...
write_xlsx(matrice_confusion_prompt_60,"matrice_confusion_prompt_60.xlsx")
