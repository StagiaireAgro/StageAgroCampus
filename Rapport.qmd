...
4.  A l’aide de la fonction `table()` on fait un tableau de fréquence en fonction des catégories croisées (le lieu d’achat croisé avec sa régularité de fréquentation).

5. A partir de ce tableau, on calcule les proportions d'individu en fonction des catégories croisées que l'on multiplie par 100 pour avoir des pourcentages à l'aide de la fonction `prop.table()`. On arrondi à deux chiffres après la virgule avec la fonction `round()`.
...
1. On sélectionne les données qui nous intéressent. On utilise la fonction `paste()` pour créer tous les noms de colonnes qui nous intéressent.

2. On regarde pour chaque individu pour chaque lieu s'il  est très fréquenté ( = 1 ou = 2). Cela nous retourne le dataframe de booléens (0 = FALSE ou 1 = TRUE) . Il nous reste à sommer par ligne à l'aide de la fonction `rowSums()`.

3. Pour afficher notre résultat, on utilise la fonction `print()` à laquel on donne comme argument notre collage avec la fonction `paste()` pour intégrer la moyenne arrondie à un chiffre après la virgule de notre moyenne du nombre de lieux très fréquenté.
```{r}
...
2.  On transforme nos données en tableau avec la fonction `table()`, puis on calcule la proportion de chaque variable (1 à 12) avec `prop.table()` que l'on va multiplier par cent pour avoir un pourcentage. On peut arrondir à deux chiffres après la virgule avec `round`.
...
1. On sélectionne des bonnes colonnes.
2. On compte le nombre de lieus régulièrement fréquenté par individu.
...
1. On compte le nombre d'occurence par nb de lieux régulièrement fréquenté avec `table()`. Et on calcul la proportion avec `prop.table()`.
2. On change le nom avec `names()` et `dimnames()`.
3. Mise en forme du tableau avec `kable()`.
4. On repréente le graphique avec `ggplot()`.
```{r}
#1

dt_nb_rgl_freq_pourcent <- round(prop.table(table(nb_rgl_freq))*100,2)
