...
1.  Selectionner le jeu de données avec les variables qui nous permette de repondre à la question posé
2.  Creer une variable qui est égale à TRUE si la personne à repondu à au moins des questions `apply()` préciser 1 pour dire que c'est en ligne (individu)
3.  Calcule la proportion de TRUE qui n'est rien d'autre que la moyenne des répondants : utiliser `mean()`
...
1.  Découper le budget en 5 catégories égales avec `quantile()` puis spécifier le pourcentage de repondant dans chaque catégorie avec `seq()`
2.  Afficher les catégories de budget avec `print()`
3.  La repartition des catégories de budget
...
1.  Spécifier les labels des modalités
2.  Recodage des modalités de la variables Q230 avec `factor()`
3.  tableau croisé dynamique `avec table()`, utiliser l'argument `useNA="ifany"` si l'on veut afficher les NA
4.  Utiliser `prop.table()` pour realiser des pourcentages dans
