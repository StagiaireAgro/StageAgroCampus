...
regrouper_produits <- function(df) {
  ...
  df <- df %>%
    mutate(
      ...
      # 2. Suppression des accents (ex : "pêche" devient "peche")
  ...
  # Détection vectorielle des mots au pluriel. Exemple : pommes, abricots etc.
  df <- df %>%
    mutate(
  ...
  for (mot_cle in mots_clefs_etendus) {
    ...
    # Si des lignes correspondent à ce mask
    if (any(mask)) {
