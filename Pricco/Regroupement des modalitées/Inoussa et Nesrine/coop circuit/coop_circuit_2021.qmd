...
export_dataframes <- function(list_of_lists_of_dataframes, annee, base_directory) {
  # Créer le répertoire de base s'il n'existe pas
  if (!dir.exists(base_directory)) {
    dir.create(base_directory, recursive = TRUE)
  }
  # Fonction pour exporter un dataframe en format .xlsx
  export_df <- function(df, name) {
    filename <- file.path(base_directory, paste0(name,"_",annee,".xlsx"))
    write_xlsx(as.data.frame(df), path = filename)
  }
  # Parcourir chaque liste de dataframes et exporter chaque dataframe
  lapply(seq_along(list_of_lists_of_dataframes), function(i) {
    inner_list <- list_of_lists_of_dataframes[[i]]
    export_df(inner_list, names(list_of_lists_of_dataframes)[i])
  })
}
