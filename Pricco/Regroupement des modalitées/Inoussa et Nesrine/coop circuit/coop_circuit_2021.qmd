...
data_coop_circuit_2021 <- vroom("data/coop circuit/par annee/2021.csv")
```
...
## 1) Stockage des levels existant
...
## 2) On regarde à l'oeil nu les variantes de noms de produits
...
## 1) Version 1 : Liste contenant toutes les déclinaisons possibles pour chaque nom de produit
...
haricot_vert_declinaison <- c("haricot vert","haricots verts", "haricot verts",
                              "haricots vert", "haricot melange vert")
...
## 2) Version 2 : Liste contenant toutes les déclinaisons possibles pour chaque nom de produit

```{r}
# Fonction : declinaisons -----------------------------------------------------
# Principe : Trouver toutes les déclinaisons possibles des noms de produits
#            présentes dans un jeu de données

nettoyage <- function(x) {
  x <- stri_trans_general(str_to_lower(x), "Latin-ASCII") # 1 & 2
  x <- str_replace_all(x, "[^a-z ]", "")                  # 3
  return(x)
}

creer_regex <- function(mots, suffixe = "(e?s)?", collapse = ".*") {
  sapply(mots, function(terme) {
    terme <- nettoyage(terme)                 # 1
    tokens <- str_split(terme, " ")[[1]]      # 2
    regex_tokens <- paste0(tokens, suffixe)   # 3
    paste0(regex_tokens, collapse = collapse) # 4
  })
}

declinaisons_noms_produits <- function(db, product_name, products) {
  
  products <- nettoyage(products)
  products_regex <- creer_regex(products)
  ordre <- order(nchar(gsub(" ", "", products)), decreasing = TRUE)
  products_regex <- products_regex[ordre]
  
  noms_des_produits <- factor(db[[product_name]])
  levels(noms_des_produits) <- nettoyage(levels(noms_des_produits))
  # Appliquer les regex aux niveaux uniques
  noms_uniques <- levels(noms_des_produits)
  
  # Extrait les termes correspondant à chaque expression régulière et 
  # les concatène en une chaîne.
  presence_mat_niveaux <- vapply(products_regex, function(prod) {
    matches <- str_extract_all(noms_uniques, prod)
    sapply(matches, function(match) {
      if (length(match) > 0) return(paste(unique(match), collapse = ","))
      else return(NA_character_) 
      }
    )
  }, FUN.VALUE = character(length(noms_uniques)))
  
  attribution <- apply(presence_mat_niveaux, 1, function(pres) {
    if (sum(!is.na(pres)) == 1) return(pres[!is.na(pres)][1])
    else if (sum(!is.na(pres)) > 1) {
      candidats <- pres[!is.na(pres)]
      premier <- candidats[1]  # Premier produit
      reste <- candidats[-1]  # Reste des produits
      if (all(sapply(reste, function(r) grepl(r, premier)))) return(premier)
      else return(NA_character_)  # Sinon, retourner NA
    }
    else return(NA_character_) 
  })
  
  result_df <- na.omit(attribution)
  return(result_df)
}

classification_produits_avec_listes <- function(vecteur, list_produits) {
  # Nettoyer le vecteur d'entrée et la liste des produits
  vecteur_nettoye <- nettoyage(vecteur)
  list_nettoyee <- nettoyage(list_produits)
  liste_regex <- creer_regex(list_nettoyee)
  ...
  niveaux <- levels(factor(vecteur_nettoye))
  # Calcul de la matrice de présence sur niveaux uniques
  ...
  fact_noms <- factor(vecteur_nettoye, levels = niveaux)
  # Reconstruction de la matrice de présence par ligne originale
  ...
  attribution <- apply(presence_mat, 1, function(pres) {
    if (sum(pres) == 1) list_produits_ord[pres][1]
    else if (sum(pres) > 1) {
      candidats <- list_nettoyee_ord[pres]
      premier <- candidats[1]
      reste <- candidats[-1]
      if (all(sapply(reste, function(r) grepl(r, premier)))) list_produits_ord[pres][1]
      else NA_character_
    } 
    else NA_character_
  })
  ...
  result_df <- data.frame(vecteur = vecteur_nettoye,produit_classe = attribution)
  # Séparer les données en ok et nok
  data_ok <- result_df[!is.na(result_df$produit_classe), ]
  # Créer la liste sous la forme souhaitée
  liste_produits_resultats <- list()
  # Parcours des produits détectés
  for (produit in unique(data_ok$produit_classe)) {
    # Extraire toutes les lignes correspondant à ce produit
    lignes_correspondantes <- data_ok[data_ok$produit_classe == produit, ]
    # Ajouter à la liste sous le nom du produit
    liste_produits_resultats[[produit]] <- unique(lignes_correspondantes$vecteur)
  }

  return(liste_produits_resultats)
}
```
...
### a) Version 1 : Avec le vecteur des variantes de noms de produits obtenu manuellement
...
### b) Version 2 : Avec le vecteur des variantes de noms de produits obtenu avec la machine

```{r}
declinaisons <- declinaisons_noms_produits(data_coop_circuit_2021, "name", liste_produits)
set.seed(123)
final_declinaisons <- classification_produits_avec_listes(
  unique(unlist(strsplit(declinaisons, ","))), liste_produits)
...
# Possibilité de supprimé les noms de la liste pour quelle ressemble à celui de Nesrine
names(final_declinaisons) <- NULL
...
```{r}
# Test de la fonction : data_ok de coop circuit 2021
liste_data_par_produit <- split_data_ok_par_produit(data_ok2)
...
"produits laitiers","parmesan","chocolat","beaudoin","cr�pes","faisselle","gwell","kefir","corporel","hydratant","le bleu","meringues","moinet","morbier","gallois","raclette","riz","saint","tablette","tisane","tomette","farine","p�te","torsade","melonnette","pain","aux noix","aperitif","miel","cookie","fumee","de noix","caramelisees","salees","sucres","pesto","torrefies","truffe","l�gumes","shampoing","tagliatelle","confit","maquereau","panier de legumes","potage","safran","gelee","creme","moelleux","pur�e","p�tillant","vinaigrette","tartelette","tapas","cake","bougie","ch�taigne","caramel","chausson","choux rouge","compote","guerzoni","petillant","s�ch�es","puree","pur�e","vinaigre","frites","gratin","p�t�","cordon","pate","paupiette","moutarde","roti","r�ti","ail","cepes","fume","herbe","noisette","poivre","tartiflette","saut�","verrine","echalion","�chalion","conserve","farce","galettes","rillettes","supreme","�chalote","pruneau","tourte","biere","bi�re","cinsault","moule","huile","mouzakis","muscat","chutney","concentre","pulpe","ketchup","basilic","apero", "ap�ro","hydrolat","cuire", "fondu","merguez","saucisson","hache",
"boulettes","tripes", "saucisses","preparation","m�lange","ap�ritif","farcir"
, "melange","m�lange","caram�lis�es","torr�fi�s","saumon","truites", "poisson","mer","jaune et rouge","a cuire","gel�e","et/ou","sac mix","m�lang�es","melangees","variees","vari�es")
```

## 3) Développement de la fonction pour supprimer les produits transformés
...
## 4) Création d'une liste pour y placer le data_jus_de_pomme
...
## 5) Création d'une liste pour y placer le data_peche
...
# On ajoute à cette liste vide le jeu de données peche
liste_peche[["peche"]] <- data_peche
```

## 6) Création d'une liste pour y placer le data_nectarine

```{r}

# On initialise une liste
liste_nectarine <- list()

# On ajoute à cette liste vide le jeu de données nectarine
liste_nectarine[["nectarine"]] <- data_nectarine

```

## 7) Retirer data_jus_de_pomme, data_nectarine et data_peche de la liste de résultat

```{r}
# On retire le jeu de données nectarine de liste_data_produits_bruts
liste_data_par_produit[["nectarine"]] <- NULL

# On retire le jeu de données peche de liste_data_produits_bruts
liste_data_par_produit[["peche"]] <- NULL

# On retire le jeu de données jus de pomme de liste_data_produits_bruts
liste_data_par_produit[["jus de pomme"]] <- NULL

```

## 8) Suppression des produits transformés pour le jus de pomme
...
## 9) Suppression des produits transformés pour les pêches

```{r}
liste_produits_transformes_peche <- c("confiture", "coulis", "jus","nectar")
```

## 10) Application de la fonction sur tous les jeux de données
...
## 11) Application de la fonction  sur les cas spécifiques
...
```{r,warning=FALSE}
# Application de la fonction sur le jeu de données peche
resultat_peche <- 
  supp_produits_transfor(liste_produits_transformes_peche, liste_peche)
```

# 12) On ne garde que les 34 jeux de données d'intérêt

```{r}
# On ne garde que les 34 jeux de données d'intérêt
liste_data_produits_bruts <- resultat[names(resultat) != "data_produits_transformes"]
```


# 13) Pour les cas spécifiques, on extrait les tables de produits bruts 
...
```{r}
# On extrait la table jus de pomme de la liste resultat_peche
data_peche <- resultat_peche[["peche"]]
```

# 14) On retire le jeu de données des produits transformés pour le cas général
...
# 15) On retire le jeu de données des produits transformés pour les cas spécifiques
...
```{r}
# On retire le jeu de données data_produits_transformes de la liste de resultat_peche
data_produits_transformes_peche<- resultat_peche$data_produits_transformes
```

# 16) On ajoute les tables spécifiques dans la liste de produits bruts final

```{r,warning=FALSE}

# On ajoute la table du jus de pomme dans notre liste de résultat
liste_data_produits_bruts[["jus de pomme"]] <- data_jus_pomme

# On ajoute la table peche dans notre liste de résultat
liste_data_produits_bruts[["peche"]] <- data_peche

# On ajoute la table nectarine dans notre liste de résultat
liste_data_produits_bruts[["nectarine"]] <- data_nectarine
```

# 17) On ajoute les lignes des tables de produits transformés dans la table data_pas_ok

```{r}
# On fusionne les 2  tables des produits transformés avec la table autre
data_pas_ok <- 
  rbind(data_pas_ok2,data_produits_transformes,data_produits_transformes_jus_pomme,
        data_produits_transformes_peche)
```
...
######### abricot
data_abricot <- liste_data_produits_bruts[["abricot"]]
...
######### carotte
data_carotte <- liste_data_produits_bruts[["carotte"]]
...
######### asperge
data_asperge <- liste_data_produits_bruts[["asperge"]]
...
######### aubergine
data_aubergine <- liste_data_produits_bruts[["aubergine"]]
...
######### banane
data_banane <- liste_data_produits_bruts[["banane"]]
...
######### boeuf
data_boeuf <- liste_data_produits_bruts[["boeuf"]]
...
######### celeri branche
data_celeri_branche <- liste_data_produits_bruts[["celeri branche"]]
...
######### cerise
data_cerise <- liste_data_produits_bruts[["cerise"]]
...
######### chou fleur
data_chou_fleur <- liste_data_produits_bruts[["chou fleur"]]
...
######### clementine
data_clementine <- liste_data_produits_bruts[["clementine"]]
...
######### concombre
data_concombre <- liste_data_produits_bruts[["concombre"]]
...
######### courgette
data_courgette <- liste_data_produits_bruts[["courgette"]]
...
######### endive
data_endive <- liste_data_produits_bruts[["endive"]]
...
######### fraise
data_fraise <- liste_data_produits_bruts[["fraise"]]
...
######### haricot vert
data_haricot_vert <- liste_data_produits_bruts[["haricot vert"]]
...
######### jus de pomme
data_jus_de_pomme <- liste_data_produits_bruts[["jus de pomme"]]
...
######### kiwi
data_kiwi <- liste_data_produits_bruts[["kiwi"]]
...
######### lait
data_lait <- liste_data_produits_bruts[["lait"]]
...
######### laitue
data_laitue <- liste_data_produits_bruts[["laitue"]]
...
######### lentille
data_lentille <- liste_data_produits_bruts[["lentille"]]
...
######### melon
data_melon <- liste_data_produits_bruts[["melon"]]
...
######### nectarine
data_nectarine <- liste_data_produits_bruts[["nectarine"]]
...
######### noix
data_noix <- liste_data_produits_bruts[["noix"]]
...
######### oeuf
data_oeuf <- liste_data_produits_bruts[["oeuf"]]
...
######### oignon
data_oignon <- liste_data_produits_bruts[["oignon"]]
...
######### peche
data_peche <- liste_data_produits_bruts[["peche"]]
...
######### poire 
data_poire <- liste_data_produits_bruts[["poire"]]
...
######### poireau 
data_poireau <- liste_data_produits_bruts[["poireau"]]
...
######### poivron 
data_poivron <- liste_data_produits_bruts[["poivron"]]
...
######### pomme 
data_pomme <- liste_data_produits_bruts[["pomme"]]
...
######### pomme de terre
data_pomme_de_terre <- liste_data_produits_bruts[["pomme de terre"]]
...
######### porc
data_porc <- liste_data_produits_bruts[["porc"]]
...
######### potiron
data_potiron <- liste_data_produits_bruts[["potiron"]]
...
######### poulet
data_poulet <- liste_data_produits_bruts[["poulet"]]
...
######### prune
data_prune <- liste_data_produits_bruts[["prune"]]
...
######### raisin
data_raisin <- liste_data_produits_bruts[["raisin"]]
...
######### tomate
data_tomate <- liste_data_produits_bruts[["tomate"]]
...
# X) Export des jeux de données

```{r}
exports_datasets(liste_data_produits_bruts, "2021", "coopcircuit")
exports_datasets <- function(datasets, annee, dossier) {
  # Crée un sous-dossier avec l'année dans le dossier principal
  sous_dossier <- file.path(dossier, as.character(annee))

  # Crée le sous-dossier, showWarnings = FALSE empêche l'affichage des avertissements si le dossier existe déjà
  dir.create(sous_dossier, showWarnings = FALSE, recursive = TRUE)

  # Fonction d'export d'un data.frame
  exporter_df <- function(df, nom_fichier) {
    # Remplace les sauts de ligne dans les colonnes de type caractère
    df[] <- lapply(df, function(col) {
      if (is.factor(col) || is.character(col)) {
        stringr::str_replace_all(col, "[\r\n]+", " ")
      } else {
        col
      }
    })
    # Chemin complet pour le fichier
    chemin <- file.path(sous_dossier, nom_fichier)
    # Écrit le fichier CSV
    write.table(df, file = chemin, sep = ";", row.names = FALSE, fileEncoding = "UTF-8")
  }

  # Vérifie si l'argument datasets est une liste de data.frames
  if (is.list(datasets) && all(sapply(datasets, is.data.frame))) {
    # Applique la fonction d'export à chaque data.frame dans la liste
    sapply(names(datasets), function(nom) {
      df <- datasets[[nom]]
      nom_fichier <- paste0(gsub("[^a-zA-Z0-9_-]", "_", nom),"_",annee, ".csv")
