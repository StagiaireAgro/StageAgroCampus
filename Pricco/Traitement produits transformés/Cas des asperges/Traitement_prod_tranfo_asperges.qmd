...
hallucinations_reelles <- pseudo_hallucinations[!sapply(pseudo_hallucinations, function(h) {
  ...
  pattern <- paste0(".*", paste(mots, collapse = ".*"), ".*")  # construit regex : .*mot1.*mot2.*mot3.*
  any(grepl(pattern, tolower(liste_attendue)))
    ...
    prod_transfo_levels_initiaux <- intersect(tolower(verite_terrain_produit_transfo), tolower(liste_attendue))
    
    produit_transfo_llm <- intersect(tolower(prod_transfo_levels_initiaux), tolower(produits_communs))

    prod_transfo_pas_trouver <- setdiff(tolower(prod_transfo_levels_initiaux), tolower(produit_transfo_llm))
