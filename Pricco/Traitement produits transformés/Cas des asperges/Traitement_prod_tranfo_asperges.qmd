...
comparer_listes_produits <- function(prompts_attendus, reponses_llm) {
  ...
  comparaison <- lapply(seq_along(produits_attendus), function(i) {
    ...
    produits_communs_initiaux <- intersect(liste_attendue, liste_extraite)
    ...
    # Ajoute aux produits_communs les pseudo_hallucinations qui ne sont pas des sous-parties des produits déjà communs
    pseudo_valides <- pseudo_hallucinations[
      !sapply(pseudo_hallucinations, function(ph) {
        any(grepl(ph, produits_communs_initiaux, fixed = TRUE))
      })
    ]

    # Fusion des produits communs initiaux avec les pseudo_hallucinations valides
    produits_communs <- c(produits_communs_initiaux, pseudo_valides)

    # Identifie les hallucinations réelles : tout ce qui reste dans produits_extraits et pseudo_hallucinations,
    # une fois les produits communs pris en compte
    hallucinations_reelles <- setdiff(unique(c(liste_extraite, pseudo_hallucinations)), produits_communs)
    ...
    list(
      ...
      pseudo_hallucinations = pseudo_hallucinations,
      hallucinations_reelles = hallucinations_reelles
    )
  })
...
table_indica_purete_asperge$nb_levels_initial <- sapply(resultats_comparaison, function(x) length(x$produits_attendus))

# On remplit la colonne du nb_levels_llm
table_indica_purete_asperge$nb_levels_llm <- sapply(resultats_comparaison, function(x) length(x$produits_extraits))

# On remplit la colonne du nb_levels_commun
table_indica_purete_asperge$nb_levels_commun <- sapply(resultats_comparaison, function(x) length(x$produits_communs))

# On remplit la colonne du nb_fausses_halluci_llm
table_indica_purete_asperge$nb_fausses_halluci_llm <- sapply(resultats_comparaison, function(x) length(x$pseudo_hallucinations))

# On remplit la colonne du nb_vraies_halluci_llm
table_indica_purete_asperge$nb_vraies_halluci_llm <- sapply(resultats_comparaison, function(x) length(x$hallucinations_reelles))
