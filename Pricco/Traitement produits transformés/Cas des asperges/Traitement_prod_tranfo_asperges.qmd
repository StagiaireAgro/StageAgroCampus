...
comparer_listes_produits <- function(prompts_attendus, reponses_llm) {
  ...
  comparaison <- lapply(seq_along(produits_attendus), function(i) {
    ...
    hallucinations_brutes <- setdiff(liste_extraite, produits_communs)
    
      # Filtrage des hallucinations : on retire celles qui sont des sous-parties d’un produit attendu
    hallucinations_filtrees <- hallucinations_brutes[!sapply(hallucinations_brutes, function(h) {
      any(grepl(h, liste_attendue, fixed = TRUE))
    })]
    ...
    list(
      ...
      pseudo_hallucinations = hallucinations_brutes,
      hallucinations_reelles = hallucinations_filtrees
    )
  })
